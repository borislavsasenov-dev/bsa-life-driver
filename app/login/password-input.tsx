  ...
  return (
    <div className="relative mt-1">
      <input
        ...
        className="w-full rounded-lg border border-stone-300 px-3 py-2 pr-10 text-sm text-stone-900 focus:border-stone-500 focus:outline-none focus:ring-1 focus:ring-stone-500"
      />
      <button
        type="button"
        onClick={() => setVisible((v) => !v)}
        aria-label={visible ? "Hide password" : "Show password"}
        className="absolute right-2 top-1/2 -translate-y-1/2 text-stone-400 transition hover:text-stone-600"
      >
        {visible ? (
